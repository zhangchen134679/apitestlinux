...
class Handler:
    ...
    data_path = os.path.join(os.path.join(path.data_path(), "data_pre"), "data_query")

    # \config\conf_prd\config_write..
    yml_conf = handler_yaml.read_yaml(os.path.join(os.path.join(path.config_path(), "conf_pre"), "config_query.yml"))

    excel_query_common = handler_excel.ExcelHandler(
        os.path.join(path.data_path(), r"data_pre\data_query\{}".format(yml_conf["excel"]["query_common"])))

    excel_query_invocation_info = handler_excel.ExcelHandler(
        os.path.join(path.data_path(), r"data_pre\data_query\{}".format(yml_conf["excel"]["query_invocation_info"])))

    excel_member_grade_calculator = handler_excel.ExcelHandler(
        os.path.join(path.data_path(), r"data_pre\data_query\{}".format(yml_conf["excel"]["member_grade_calculator"])))
    ...
    def replace_data(self, brand_code, data):
        ...
        while re.search(pattern, data):
            ...
            if key in ("tradeNo", "oriOrderId"):
                data = re.sub(pattern, str(self.crm_trade_no(brand_code, channel="online")["trade_no"]), data, count=1)
            else:
