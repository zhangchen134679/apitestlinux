...
class Handler:
    ...
    yml_conf = handler_yaml.read_yaml(os.path.join(path.config_path(), r"conf_pre_apitest_back\config_query.yml"))

    # 加载excel配置项
    __excel_config = yml_conf["excel"]
    # 实例化handler_excel
    excel_kie = handler_excel.ExcelHandler(
        os.path.join(path.data_path(), r"data_pre_apitest_back\data_query\{}".format(__excel_config["kie"])))

    excel_lrp = handler_excel.ExcelHandler(
        os.path.join(path.data_path(), r"data_pre_apitest_back\data_query\{}".format(__excel_config["lrp"])))
...
def register_member(brand_code, headers, chanel_code="986"):
    ...
    member = handler_requests.visit(url=Handler.yml_conf["host"] + "/member/register",
                                    ...
                                    headers=headers)
    try:
        if member["code"] != 0 or "写入操作被拦截" in member["data"]["remark"]:
