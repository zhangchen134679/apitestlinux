...
excel = handler_middle.Handler().excel_upload_change_grade_SGMB

cases = excel.read_data("sgmb_upload_order_verifyGrade")
...
class TestUploadOrderVerifyGrade(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        # 从配置文件读取brand
        cls.brand = handler_middle.Handler.yml_conf["brand"]["sgmb"]
        ...
        cls.order_158_back_100 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.order_58_back_58 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.order_200_back_200 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
...
# --------------------------------------------------------------------------------------------------------------------
        # 替换用例中降级用到的tradeNo
        data_info = re.sub(r"&order_158_back_100&", self.order_158_back_100, str(data_info))
        data_info = re.sub(r"&order_58_back_58&", self.order_58_back_58, str(data_info))
        data_info = re.sub(r"&order_200_back_200&", self.order_200_back_200, str(data_info))
