...
excel = handler_middle.Handler().excel_upload_change_grade_MBB

cases = excel.read_data("month25")

"""用于需要调用订单信息更新接口"""
brand = handler_middle.Handler.yml_conf["brand"]["mbb"]
...
class TestUploadOrderVerifyGrade(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        # 从配置文件读取brand
        cls.brand = handler_middle.Handler.yml_conf["brand"]["mbb"]
        ...
        # 调用方法注册品牌下的会员
        for i in range(0, 10):
            member = handler_middle.register_member(cls.brand,birthday="1995-04-09", headers=cls.headers)
            cls.member_list.append(member["data"]["union_code"])

        # 调用方法注册品牌下的会员生日月

        for i in range(0, 10):
            member = handler_middle.register_member(cls.brand,birthday="1995-01-09", headers=cls.headers)
