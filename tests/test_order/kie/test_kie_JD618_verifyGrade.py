...
cases = excel.read_data("kie_JD_order_verifyGrade")
...
class TestUploadOrderVerifyGrade(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        # 调用方法注册品牌下的生日月会员
        for i in range(0, 3):
            member = handler_middle.register_member(cls.brand, birthday="1995-05-01", headers=cls.headers)
            cls.member_list_birthday.append(member["data"]["union_code"])

        # 用例中降级用到的tradeNo
        cls.test_order_back = "test_arvAto_back_".lower()
        cls.order_555_back_555 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.order_2040_back_1020 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.order_970_back_970 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.order_1040_back_1040 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.order_955_back_955 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.order_1035_back_1035 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
...
# --------------------------------------------------------------------------------------------------------------------
        # 替换用例中降级用到的tradeNo
        data_info = re.sub(r"&order_555_back_555&", self.order_555_back_555, str(data_info))
        data_info = re.sub(r"&order_2040_back_1020&", self.order_2040_back_1020, str(data_info))
        data_info = re.sub(r"&order_970_back_970&", self.order_970_back_970, str(data_info))
        data_info = re.sub(r"&order_1040_back_1040&", self.order_1040_back_1040, str(data_info))
        data_info = re.sub(r"&order_955_back_955&", self.order_955_back_955, str(data_info))
        data_info = re.sub(r"&order_1035_back_1035&", self.order_1035_back_1035, str(data_info))
