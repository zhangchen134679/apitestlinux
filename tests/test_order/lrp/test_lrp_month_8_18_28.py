...
excel = handler_middle.Handler().excel_upload_change_grade_lrp

cases = excel.read_data("lrp_monthly")
...
class TestUploadOrderVerifyGrade(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        # 调用方法注册品牌下的生日月会员
        for i in range(0, 2):
            member = handler_middle.register_member(cls.brand, birthday="1995-02-01", headers=cls.headers)
        ...
        cls.test_order_1000_back_500 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
...
# --------------------------------------------------------------------------------------------------------------------
        ...
        data_info = re.sub(r"&test_order_1000_back_500&", self.test_order_1000_back_500, str(data_info))
# --------------------------------------------------------------------------------------------------------------------
        # 替换用例中的data数据..
...
#  --------------------------------------------------------------------------------------------------------------------
        ...
        if data['orderType'] ==1:
            ...
            response = handler_middle.upload_order_info_new(self.brand,data['tradeNo'],self.headers)
        ...
        try:
            ...
            if expect_response_point == '无积分':
                ...
                logger.info("积分为：{}".format(aaa['data']['points'][0]['evalidPoints']))
