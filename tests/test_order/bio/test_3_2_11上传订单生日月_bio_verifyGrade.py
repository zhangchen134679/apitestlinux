...
class TestUploadOrderVerifyGrade(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        # 调用方法注册品牌下的会员
        for i in range(0, 20):
            member = handler_middle.register_member(cls.brand, headers=cls.headers,birthday="1995-02-09")
            cls.member_list.append(member["data"]["union_code"])

        # 用例中降级用到的tradeNo
        cls.test_order_back = "test_arvAto_back_".lower()
        cls.tradeNo_1_10000 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.tradeNo_2_1000 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.tradeNo_4_10000 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.tradeNo_5_1000 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.tradeNo_7_10000 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.tradeNo_8_1000 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.tradeNo_10_10000 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.tradeNo_11_1000 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.tradeNo_13_10000 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.tradeNo_14_1000 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
...
# --------------------------------------------------------------------------------------------------------------------
        # 替换用例中降级用到的tradeNo
        data_info = re.sub(r"&tradeNo_1_10000&", self.tradeNo_1_10000, str(data_info))
        data_info = re.sub(r"&tradeNo_2_1000&", self.tradeNo_2_1000, str(data_info))
        data_info = re.sub(r"&tradeNo_4_10000&", self.tradeNo_4_10000, str(data_info))
        data_info = re.sub(r"&tradeNo_5_1000&", self.tradeNo_5_1000, str(data_info))
        data_info = re.sub(r"&tradeNo_7_10000&", self.tradeNo_7_10000, str(data_info))
        data_info = re.sub(r"&tradeNo_8_1000&", self.tradeNo_8_1000, str(data_info))
        data_info = re.sub(r"&tradeNo_10_10000&", self.tradeNo_10_10000, str(data_info))
        data_info = re.sub(r"&tradeNo_11_1000&", self.tradeNo_11_1000, str(data_info))
        data_info = re.sub(r"&tradeNo_13_10000&", self.tradeNo_13_10000, str(data_info))
        data_info = re.sub(r"&tradeNo_14_1000&", self.tradeNo_14_1000, str(data_info))
