...
excel = handler_middle.Handler().excel_upload_change_grade_KS

cases = excel.read_data("ks_upload_order_verifyGrade")
...
class TestUploadOrderVerifyGrade(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        # 从配置文件读取brand
        cls.brand = handler_middle.Handler.yml_conf["brand"]["ks"]
        ...
        cls.order_2501_back_1000 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.order_2501_back_1800 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.order_2501_back_2501 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.order_1501_back_800 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.order_1501_back_1501 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.order_701_back_701 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
...
# --------------------------------------------------------------------------------------------------------------------
        # 替换用例中降级用到的tradeNo
        data_info = re.sub(r"&test_order_2501_back_1000&", self.order_2501_back_1000, str(data_info))
        data_info = re.sub(r"&test_order_2501_back_1800&", self.order_2501_back_1800, str(data_info))
        data_info = re.sub(r"&test_order_2501_back_2501&", self.order_2501_back_2501, str(data_info))
        data_info = re.sub(r"&test_order_1501_back_800&", self.order_1501_back_800, str(data_info))
        data_info = re.sub(r"&test_order_1501_back_1501&", self.order_1501_back_1501, str(data_info))
        data_info = re.sub(r"&test_order_701_back_701&", self.order_701_back_701, str(data_info))
