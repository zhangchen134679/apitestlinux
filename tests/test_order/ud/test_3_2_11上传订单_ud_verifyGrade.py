...
excel = handler_middle.Handler().excel_upload_change_grade_UD

cases = excel.read_data("ud_upload_order_verifyGrade")
...
class TestUploadOrderVerifyGrade(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        # 从配置文件读取brand
        cls.brand = handler_middle.Handler.yml_conf["brand"]["ud"]
        ...
        cls.order_700_back_500 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
    ...
    def test_upload_order_verifyGrade(self, data_info):
        ...
        # --------------------------------------------------------------------------------------------------------------------
        # 替换用例中降级用到的tradeNo
        data_info = re.sub(r"&order_700_back_500&", self.order_700_back_500, str(data_info))
