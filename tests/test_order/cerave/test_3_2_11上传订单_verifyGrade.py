...
excel = handler_middle.Handler().excel_upload_change_grade_CERAVE

cases = excel.read_data("cerave_upload_order_verifyGrade")
...
class TestUploadOrderVerifyGrade(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        # 从配置文件读取brand
        cls.brand = handler_middle.Handler.yml_conf["brand"]["cerave"]
        ...
        cls.order_1200_back_600 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.order_1200_back_900 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.order_1200_back_1160 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.order_1200_back_1200 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.order_600_back_300 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.order_600_back_560 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.order_600_back_600 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.order_300_back_260 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.order_300_back_300 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.order_40_back_40 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
...
# --------------------------------------------------------------------------------------------------------------------
        # 替换用例中降级用到的tradeNo
        data_info = re.sub(r"&test_order_1200_back_600&", self.order_1200_back_600, str(data_info))
        data_info = re.sub(r"&test_order_1200_back_900&", self.order_1200_back_900, str(data_info))
        data_info = re.sub(r"&test_order_1200_back_1160&", self.order_1200_back_1160, str(data_info))
        data_info = re.sub(r"&test_order_1200_back_1200&", self.order_1200_back_1200, str(data_info))
        data_info = re.sub(r"&test_order_600_back_300&", self.order_600_back_300, str(data_info))
        data_info = re.sub(r"&test_order_600_back_560&", self.order_600_back_560, str(data_info))
        data_info = re.sub(r"&test_order_600_back_600&", self.order_600_back_600, str(data_info))
        data_info = re.sub(r"&test_order_300_back_260&", self.order_300_back_260, str(data_info))
        data_info = re.sub(r"&test_order_300_back_300&", self.order_300_back_300, str(data_info))
        data_info = re.sub(r"&test_order_40_back_40&", self.order_40_back_40, str(data_info))
# --------------------------------------------------------------------------------------------------------------------
        # 替换用例中的data数据..
