...
excel = handler_middle.Handler().excel_upload_change_grade_skc

cases = excel.read_data("skc_upload_order_verifyGrade")
...
class TestUploadOrderVerifyGrade(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        # 从配置文件读取brand
        cls.brand = handler_middle.Handler.yml_conf["brand"]["skc"]
        ...
        cls.test_order_10000_back_9000 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.test_order_10000_back_8000 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.test_order_10000_back_5000 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.test_order_5000_back_3000 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.test_order_5000_back_1000 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
...
# --------------------------------------------------------------------------------------------------------------------
        # 替换用例中降级用到的tradeNo
        data_info = re.sub(r"&test_order_10000_back_9000&", self.test_order_10000_back_9000, str(data_info))
        data_info = re.sub(r"&test_order_10000_back_8000&", self.test_order_10000_back_8000, str(data_info))
        data_info = re.sub(r"&test_order_10000_back_5000&", self.test_order_10000_back_5000, str(data_info))
        data_info = re.sub(r"&test_order_5000_back_3000&", self.test_order_5000_back_3000, str(data_info))
        data_info = re.sub(r"&test_order_5000_back_1000&", self.test_order_5000_back_1000, str(data_info))
...
#  --------------------------------------------------------------------------------------------------------------------
        ...
        # 增加积分查询
            actual_point = handler_middle.query_member_points(self.brand,data["channelMemberId"],self.headers)

            # logger.info("查询积分接口返回: {}".format(actual_point))
            logger.info("查询积分接口返回的积分: {}".format(actual_point['data']['points'][0]['evalidPoints']))
