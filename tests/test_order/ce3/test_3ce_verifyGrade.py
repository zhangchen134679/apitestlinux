...
excel = handler_middle.Handler().excel_upload_change_grade_3CE

cases = excel.read_data("3ce_upload_order_verifyGrade")
...
class TestUploadOrderVerifyGrade(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        # 从配置文件读取brand
        cls.brand = handler_middle.Handler.yml_conf["brand"]["3ce"]
        ...
        cls.order_601_back_1 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.order_1201_back_1 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.order_200_back_1 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.order_301_back_1 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.order_701_back_1 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.order_901_back_1 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
        cls.order_201_back_1 = f"{cls.test_order_back}{handler_middle.Handler().random_num}"
    ...
    def test_upload_order_verifyGrade(self, data_info):
        ...
        data_info = re.sub(r"&order_601_back_1&", self.order_601_back_1, str(data_info))
        data_info = re.sub(r"&order_1201_back_1&", self.order_1201_back_1, str(data_info))
        data_info = re.sub(r"&order_200_back_1&", self.order_200_back_1, str(data_info))
        data_info = re.sub(r"&order_301_back_1&", self.order_301_back_1, str(data_info))
        data_info = re.sub(r"&order_701_back_1&", self.order_701_back_1, str(data_info))
        data_info = re.sub(r"&order_901_back_1&", self.order_901_back_1, str(data_info))
        data_info = re.sub(r"&order_201_back_1&", self.order_201_back_1, str(data_info))
        # 当前时间减一天进行替换
        data_info = re.sub(r"&date_time_1&",
