...
cases = excel.read_data("query_points_expire_range")


@pytest.mark.lrp
@ddt.ddt
class TestQueryPointsExpireRange(unittest.TestCase):
    ...
    @allure.step("查询最近区间过期积分")
    @pytest.mark.query
    @pytest.mark.query_points_expire_range
    @ddt.data(*cases)
    def test_query_points_expire_range(self, data_info):

        # 替换用例中的brand_code
        data_info = re.sub(r"#brand_code#", self.brand, str(data_info))
#  --------------------------------------------------------------------------------------------------------------------
        # 替换用例中没有积分的union_code
        data_info = re.sub(r"#not_points_union_code#", self.union_code, str(data_info))
#  --------------------------------------------------------------------------------------------------------------------
        # 替换用例中的value值
