...
class TestMemberUpdate(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        # 从配置文件读取deal_store_no... 用来调用方法时传入的store_code参数
        cls.store_code = handler_middle.Handler.yml_conf["crm_store"][cls.brand]["deal_store_no"]

        # 从配置文件读取replace_deal_store_no... 用来替换用例数据时传入的store_code参数
        ...
        # 查询注册会员基本信息
        cls.query_member = handler_middle.query_member_info(cls.brand, value=cls.union_code, headers=cls.headers)

        # 提取注册会员的mobile...
        """用来替换用例中的手机号..验证更新时用注册手机号update"""
        cls.register_mobile = cls.query_member["data"]["mobile"]
...
# --------------------------------------------------------------------------------------------------------------------
        # 替换用例中的原注册会员mobile..update
        data_info = re.sub(r"#register_mobile#", self.register_mobile, str(data_info))
# --------------------------------------------------------------------------------------------------------------------
        # 替换用例中的union_code..card_no
...
#  --------------------------------------------------------------------------------------------------------------------
        # 替换用例中的store_code
        data_info = re.sub("#store_code#", self.replace_store_code, str(data_info))
# --------------------------------------------------------------------------------------------------------------------
        # 转化数据格式
