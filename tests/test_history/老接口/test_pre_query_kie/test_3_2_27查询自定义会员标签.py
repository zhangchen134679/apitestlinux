...
cases = excel.read_data("query_define_member_tags")


@ddt.ddt
class TestSetDefineTags(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        # 调用设置自定义会员标签接口
        cls.tags = handler_middle.set_define_member_tags(
                    cls.brand, union_code=cls.conf["union_code"], source_tag=cls.source_tag,
                    key="hobby", value="Python", headers=cls.headers)

    @ddt.data(*cases)
    def test_query_define_member_tags(self, data_info):
...
# --------------------------------------------------------------------------------------------------------------------
        # 替换用例中的sourceTag
        data_info = re.sub(r"#sourceTag#", self.source_tag, str(data_info))
# --------------------------------------------------------------------------------------------------------------------
        # 转化数据格式
...
#  --------------------------------------------------------------------------------------------------------------------
        ...
        try:
            ...
            for self.k, self.v in expect_response.items():
                if self.k in "data" and data_info["case_id"] not in (8, 9):
                    ...
                    try:
                        ...
                        print("预期结果: {}, 实际结果: {}".format(self.value, actual[self.k][self.key]))
                        raise err

                else:
                    print("预期结果: {}, 实际结果: {}".format(self.v, actual[self.k]))
