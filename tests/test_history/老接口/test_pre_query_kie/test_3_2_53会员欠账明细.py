...
cases = excel.read_data("query_member_arrears_detail")


@ddt.ddt
class TestQueryMemberArrearsDetail(unittest.TestCase):
    ...
    @ddt.data(*cases)
    def test_query_member_arrears_detail(self, data_info):

        # 替换用例中的brand_code
        data_info = re.sub(r"#brand_code#", self.brand, str(data_info))
#  --------------------------------------------------------------------------------------------------------------------
        # 替换用例中没有欠账明细的union_code
        data_info = re.sub(r"#not_arrears_union_code#", self.register_member["data"]["union_code"], str(data_info))
#  --------------------------------------------------------------------------------------------------------------------
        # 替换用例中的value值
            ...
            for self.k, self.v in expect_response.items():
                if self.k in "data":
                    ...
                    try:
                        ...
                        # self.key == total..arrearsPointSum..payPointSum..rows
                        for self.key, self.value in self.v.items():

                            if self.key in "rows":
                                for i, j in enumerate(expect_response[self.k][self.key]):

                                    try:
                                        logger.info("第{}条用例threeAssert...".format(data_info["case_id"]))
                                        # self.keys == id..memberCode..createDate..createTime..status
                        ...
                        logger.error(
                            "againAssert...预期结果: {}, 实际结果: {}".format(self.value, actual[self.k][self.key]))
