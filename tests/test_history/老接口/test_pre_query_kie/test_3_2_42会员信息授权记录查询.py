                        ...
                        for self.key, self.value in self.v.items():
                            if self.key in "details":
                                ...
                                try:
                                    ...
                                    # self.keys == memberCode..createTime..consentStatus
                                    for self.keys, self.values in self.value[0].items():
                                        # print(self.values, actual[self.k][self.key][0][self.keys])
                                        self.assertTrue(self.values == actual[self.k][self.key][0][self.keys])

                                except AssertionError as err:
                                    logger.error("threeAssert...预期结果: {}, 实际结果: {}".
                                                 format(self.values, actual[self.k][self.key][0][self.keys]))
