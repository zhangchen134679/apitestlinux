...
# --------------------------------------------------------------------------------------------------------------------
        ...
        try:
            ...
            for self.k, self.v in eval(data_info["expect_response"]).items():
                ...
                if self.k in "data":
                    actual_data = actual[self.k][0]

                    try:
                        logger.info("第{}条用例againAssert...".format(data_info["case_id"]))
                        for self.key, self.value in self.v[0].items():
                            self.assertTrue(self.value == actual_data[self.key])

                    except AssertionError as err:
                        logger.error("againAssert...预期结果: {}, 实际结果: {}".format(self.value, actual_data[self.key]))
                        print("againAssert...预期结果: {}, 实际结果: {}".format(self.value, actual_data[self.key]))
                        raise err
