...
class TestMemberUpdate(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        :param class_tb_mobile: 提取会员mobile..
               ...
               """
        # 调用会员注册接口
        cls.class_tb_register = handler_middle.register_member(cls.brand, headers=cls.headers, chanel_code="976")

        # 提取会员union_code
        cls.class_tb_union_code = cls.class_tb_register["data"]["union_code"]

        # 查询注册会员基本信息
        cls.query_member = handler_middle.query_member_info(cls.brand, value=cls.class_tb_union_code,
                                                            headers=cls.headers)
        ...
        try:
            ...
            for self.k, self.v in expect_response.items():
                ...
                if self.k in "expect_member_info":
                    ...
                    print("会员信息：" + str(self.actual_member_info))
                    try:
                        address = "address"
                ...
                # if data_info['case_id'] in (26, 27, 28):
                #     self.actual_member_info = handler_middle.query_member_info(
                #         self.brand, value=data["union_code"], headers=self.headers)
