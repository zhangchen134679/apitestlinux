...
cases = excel.read_data("query_member_order")


@pytest.mark.lrp
@ddt.ddt
class TestQueryMemberOrder(unittest.TestCase):
    ...
    @allure.step("订单列表查询")
    @pytest.mark.query
    @pytest.mark.query_member_order
    @ddt.data(*cases)
    def test_query_member_order(self, data_info):
            ...
            for self.k, self.v in expect_response.items():
                ...
                if self.k in "data" and actual["code"] == 0:
                    for self.key, self.value in self.v.items():
                        ...
                        if self.key in "orders" and data_info["case_id"] != 29:
                        ...
                        # 会员orders数据为空
                        elif data_info["case_id"] == 29:
                            self.assertTrue(not actual[self.k]["orders"])
