...
cases = excel.read_data("query_member_Info")
...
#  --------------------------------------------------------------------------------------------------------------------
        ...
        expect_response = json.loads(data_info["{}_expect_response".format('lancome')])
        print('预期------------'+str(expect_response))
            ...
            for self.k, self.v in expect_response.items():
                ...
                if self.k in "data" and actual["code"] == 0:
                    for self.key, self.value in self.v.items():
                        if self.key in "media_account":
                            for index, value in enumerate(self.value):
                                # print(index, value)
                                for self.keys, self.values in self.value[index].items():
                                    # print(self.values, actual[self.k][self.key][index][self.keys])
                                    self.assertEqual(self.values, actual[self.k][self.key][index][self.keys])
