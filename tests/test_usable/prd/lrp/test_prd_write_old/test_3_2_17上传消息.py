...
excel = handler_middle.Handler().excel_upload_message_info
...
# --------------------------------------------------------------------------------------------------------------------
        ...
        try:
            del actual['data']['union_code']
            del expect_response['data']['union_code']
        except:
            pass
        try:
            ...
            for self.k, self.v in expect_response.items():
                ...
                if self.k in "details":
                    ...
                    try:
                        # 积分明细total == 1
                        self.assertEqual(len(actual_points_detail["data"][self.k]), 1)
