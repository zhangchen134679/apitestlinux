...
excel = handler_middle.Handler().excel_bio

cases = excel.read_data("query_member_info_consent_log")


@pytest.mark.bio
@ddt.ddt
class TestQueryMemberInfoConsentLog(unittest.TestCase):
    ...
    @allure.step("会员信息授权记录查询")
    @pytest.mark.query
    @pytest.mark.query_member_info_consent_log
    @ddt.data(*cases)
    def test_query_member_info_consent_log(self, data_info):

        # 替换用例中的brand_code
        data_info = re.sub(r"&rbrand_code&", self.brand, str(data_info))
#  --------------------------------------------------------------------------------------------------------------------
        # 替换用例中的value值
                    ...
                    for self.key, self.value in self.v.items():
                        ...
                        if self.key in "details":
                            for self.keys, self.values in self.value[0].items():
                                # print(self.values, actual[self.k][self.key][0][self.keys])
                                self.assertEqual(self.values, actual[self.k][self.key][0][self.keys])
