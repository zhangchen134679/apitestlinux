...
cases = excel.read_data("query_orderHistory_info")
...
@pytest.mark.hr
@ddt.ddt
class TestMemberQueryOrderHistoryInfo(unittest.TestCase):
    ...
    @allure.step("...")
    @pytest.mark.write
    @pytest.mark.query_orderHistory_info
    @ddt.data(*cases)
    def test_query_orderHistory_info(self, data_info):
        ...
        # 替换用例中的tradeTime
        data_info = re.sub("&trade_time&",time.strftime("%Y-%m-%d"), str(data_info))
